    ...
    <section class="rounded-2xl border border-red-200 bg-red-50 p-5 dark:border-red-950 dark:bg-slate-950">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div class="flex shrink-0 flex-wrap gap-2">
                <a href="{{ route('project-progress.monitoring-tool', $preparedReport) }}" class="inline-flex items-center justify-center rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-xs font-bold text-gray-900 shadow-sm hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 focus-visible:ring-offset-2 dark:border-slate-600 dark:bg-slate-900 dark:text-white dark:hover:bg-slate-800">Download Prepared PDF</a>
                <form method="POST" action="{{ route('project-progress.submit-prepared', [$topic, $preparedReport]) }}">
                    @csrf
                    <button class="inline-flex items-center justify-center rounded-xl bg-gray-950 px-4 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 focus-visible:ring-offset-2 dark:bg-white dark:text-gray-950 dark:hover:bg-gray-200">Submit to Research Head</button>
                </form>
                <form method="POST" action="{{ route('project-progress.discard-prepared', [$topic, $preparedReport]) }}" onsubmit="return confirm('Discard this prepared PDF? You will need to prepare it again.')">
                    @csrf
                    @method('DELETE')
                    <button class="inline-flex items-center justify-center rounded-xl border border-red-200 bg-white px-4 py-2.5 text-xs font-bold text-red-700 shadow-sm hover:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 focus-visible:ring-offset-2">Discard</button>
                </form>
            </div>
        </div>
    </section>
    ...
    >
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-5">
            ...
            <div class="flex flex-wrap gap-2">
                <button type="button" @click="generatePreview" :disabled="previewLoading || submitting" class="rounded-xl border border-gray-300 bg-white px-5 py-3 text-xs font-bold text-gray-900 shadow-sm hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 focus-visible:ring-offset-2 disabled:cursor-wait disabled:opacity-60 dark:border-slate-600 dark:bg-slate-900 dark:text-white dark:hover:bg-slate-800">
                    <span x-show="!previewLoading">Preview Monitoring Tool</span>
                    <span x-show="previewLoading" x-cloak>Generating preview…</span>
                </button>
                <button type="submit" :disabled="submitting || previewLoading" class="rounded-xl bg-red-700 px-5 py-3 text-xs font-bold text-white shadow-sm hover:bg-red-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 focus-visible:ring-offset-2 disabled:cursor-wait disabled:opacity-60">
