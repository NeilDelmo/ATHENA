...
<section class="cover"><h1>TERMINAL REPORT</h1><h2>I. Cover Page</h2><h2>Batangas State University</h2><p>The National Engineering University</p><h1>{{ $title }}</h1><p>{{ $report->researchers }}</p><p>{{ $date($report->implementation_start) }} – {{ $date($report->implementation_end) }}</p><p>Approved funding: PHP {{ number_format((float) $report->budget, 2) }}</p></section>
