                ...
                @forelse ($topics as $topic)
                    ...
                    <article class="grid gap-5 p-5 xl:grid-cols-[minmax(0,1fr)_380px]">
                        <div class="min-w-0">
                            ...
                            <div class="mt-4 flex flex-wrap gap-2"><a href="{{ route('topics.show', $topic) }}{{ $topic->status === 'approved' ? '#project-monitoring' : '' }}" class="inline-flex rounded-xl bg-gray-900 px-3 py-2 text-xs font-bold text-white">{{ $topic->status === 'approved' ? 'Open monitoring' : 'Open review workspace' }}</a><a href="{{ route('topics.download', $topic) }}" class="inline-flex rounded-xl border border-gray-200 px-3 py-2 text-xs font-bold text-gray-700">Download latest proposal</a></div>
                            @if ($topic->status === 'approved')<div class="mt-4 rounded-xl bg-blue-50 p-3 text-xs font-bold text-blue-800">{{ ucfirst($topic->project_status ?: 'ongoing') }} · {{ $topic->progressReports->count() }} progress {{ Str::plural('report', $topic->progressReports->count()) }} · {{ $topic->progressReports->where('review_status', 'pending')->count() }} awaiting review</div>@endif
                        ...
                        <div>
                            @if ($canDecide)
                                <form action="{{ route('research_head.topics.updateStatus', $topic) }}" method="POST" enctype="multipart/form-data" class="space-y-3 rounded-2xl border border-gray-200 bg-gray-50 p-4">@csrf @method('PATCH')<input type="hidden" name="reviewing_topic_id" value="{{ $topic->id }}">
                                    @if ($screeningTemplates->isNotEmpty())<div class="rounded-xl bg-purple-50 p-3 text-xs leading-5 text-purple-800"><p class="font-black">Official Initial Screening form</p><div class="mt-2 flex flex-wrap gap-2">@foreach ($screeningTemplates as $template)<a href="{{ route('proposal-templates.download', $template) }}" class="rounded-lg bg-purple-700 px-3 py-2 text-[11px] font-bold text-white">Download form</a>@endforeach</div></div>@endif
