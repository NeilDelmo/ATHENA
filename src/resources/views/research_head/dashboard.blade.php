<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm leading-6 text-gray-600">Open a submitted proposal, review its files, then upload the external evaluation with your decision.</p>
        </div>
    </x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
            @foreach ([
                ...
                ['Awaiting review', $summary['awaiting_review'], 'text-amber-700 bg-amber-50', null],
                ['Revision requested', $summary['revision_requested'], 'text-blue-700 bg-blue-50', null],
                ['Ready for signature', $summary['ready_for_signature'], 'text-red-700 bg-red-50', null],
                ['Approved projects', $summary['approved'], 'text-green-700 bg-green-50', route('research_head.projects.index')],
                ['Rejected', $summary['rejected'], 'text-red-700 bg-red-50', null],
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                    ...
                    @if ($url)
                        <a href="{{ $url }}" class="mt-3 block text-sm font-bold text-green-700">Open project monitoring &rarr;</a>
                    @endif
                </div>
            @endforeach
        </div>
        ...
        <form method="GET" action="{{ route('research_head.dashboard') }}" class="grid gap-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:grid-cols-[1fr_220px_auto]">
            ...
            <div class="flex gap-2">
                <button class="rounded-xl bg-red-600 px-4 py-2 text-sm font-bold text-white">Filter</button>
                @if ($search !== '' || $status !== '')
                    <a href="{{ route('research_head.dashboard') }}" class="inline-flex items-center rounded-xl border border-gray-200 px-4 py-2 text-sm font-bold text-gray-600">Clear</a>
                @endif
            </div>
        </form>
        ...
        <section aria-labelledby="received-proposals-heading" class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div class="border-b border-gray-100 p-5">
                ...
                <p class="mt-1 text-sm text-gray-600">Open a submission to review the package and record your decision.</p>
            </div>
            ...
            <div class="divide-y divide-gray-100">
                @forelse ($topics as $topic)
                    @php
                        ...
                        $statusStyle = match ($topic->status) {
                            ...
                            'expert_review', 'for_final_decision', 'resubmitted' => 'bg-amber-50 text-amber-800',
                    ...
                    <article class="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-center">
                        <div class="min-w-0">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span class="rounded-full px-2.5 py-1 text-xs font-black {{ $statusStyle }}">{{ $statusLabel }}</span>
                            </div>
                            <p class="mt-1 text-sm font-semibold text-gray-600">{{ $topic->researchCall->title }}@if ($topic->category) &middot; {{ $topic->category->name }}@endif</p>
                            ...
                            @if ($latestReview?->comment)
                                <div class="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
                                    <p class="text-xs font-black uppercase tracking-wider text-amber-700">Latest review feedback</p>
                                    <p class="mt-1 whitespace-pre-line text-sm leading-6 text-amber-900">{{ $latestReview->comment }}</p>
                                    @if ($topic->status === 'revision_requested')
                                        <p class="mt-2 text-xs font-black uppercase tracking-wider text-blue-700">Waiting for faculty revision</p>
                                    @endif
                                </div>
                            @endif
                            <dl class="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm text-gray-600">
                        ...
                        <div class="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <p class="text-xs font-black uppercase tracking-wider text-gray-500">Latest submission</p>
                                    <p class="mt-1 text-sm font-black text-gray-900">{{ $latestFiles->where('document_type', '!=', \App\Models\ProposalVersionFile::TYPE_HEAD_UPLOAD)->count() }} files received</p>
                                </div>
                                @if ($latestVersion)
                                    <span class="rounded-full bg-white px-2.5 py-1 text-xs font-black text-gray-600 shadow-sm">v{{ $latestVersion->version_number }}</span>
                                @endif
                            </div>
                            <p class="mt-2 text-sm leading-6 text-gray-600">{{ $latestVersion ? $latestVersion->created_at->format('M j, Y g:i A') : 'No submitted version available' }}</p>

                            <a href="{{ route('topics.show', $topic) }}#proposal-review" class="mt-4 inline-flex w-full items-center justify-center rounded-xl bg-gray-900 px-4 py-3 text-sm font-bold text-white transition hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
                                {{ match ($topic->status) {
                                    'approved', 'rejected' => 'Open proposal record',
                                    'ready_for_signature' => 'Complete signatures',
                ...
                @empty
                    <div class="p-12 text-center">
                        ...
                        <p class="mt-1 text-sm text-gray-500">Try changing the search or status filter.</p>
                    </div>
                @endforelse
            </div>
