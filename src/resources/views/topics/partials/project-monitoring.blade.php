    ...
    <div class="space-y-5 p-6">
        ...
        @else
            @if ($topic->isAccessibleTo(Auth::user()) && $projectStatus !== 'completed')
                <div class="space-y-3">
                    <x-monitoring-tool-form :topic="$topic" :prepared-report="$preparedProgressReport" :revision-report="$revisionProgressReport" :monitoring-draft="$monitoringDraft" />
                    <x-progress-report-form :topic="$topic" :prepared-report="$preparedNarrativeReport" :narrative-report-draft="$narrativeReportDraft" />
                </div>
            @endif
        @endif
                ...
                <article id="monitoring-tool-{{ $report->id }}" class="scroll-mt-32 rounded-xl border border-gray-200 p-4">
                    <div class="flex flex-wrap justify-between gap-3">
                        <div>
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span class="rounded-full px-2 py-1 text-[10px] font-black uppercase {{ $isCurrentVersion ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-500' }}">{{ $isCurrentVersion ? 'Current submission' : 'Historical version' }}</span>
                            </div>
                    ...
                    <div class="mt-3 flex flex-wrap gap-4">
                        ...
                        @if (! Auth::user()->isUsingWorkspace('research_head') && $isCurrentVersion && $report->review_status === 'revision_requested')
                            <a href="{{ route('topics.show', ['topic' => $topic, 'revise_monitoring_report' => $report->id]).'#monitoring-tool-'.$report->id }}" class="inline-flex text-xs font-bold text-red-700">Revise this {{ $report->quarter_label }} submission</a>
                        @endif
                    </div>
